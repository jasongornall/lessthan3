...
input_file='test.grep'
#input_file='input'
############### BASIC PARSING
global_data={'pageviews':[],'pings':[],'requests':[]}
f = open(input_file, 'r')
progress=-1
for line in f:
		pprint(line)
		...
		if formatter.isPingData(line):
			...
			global_data['pings'].extend(data)
		elif formatter.isRequestData(line):
			data=formatter.formatRequestData(line)
			global_data['requests'].extend(data)
		elif formatter.isPageViewData(line):
			data=formatter.formatPageViewData(line)
			global_data['pageviews'].extend(data)
	
		
		print ("----------progress ", progress)
		
prettyString=json.dumps(global_data, sort_keys=True, indent=4, separators=(',', ': '))
w = open('output', 'w+')
